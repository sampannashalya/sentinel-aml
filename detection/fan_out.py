...
class FanOutDetector:
    ...
    def __init__(
        ...
        self.default_parameters = default_parameters or {
            "min_distinct_receivers": 5,
            "time_window_hours": 24,
            "min_transaction_count": 5,
        }
        self.metadata = ToolMetadata(
            name="fan_out_detector",
            description="Detect accounts sending to many distinct receivers within a configurable time window.",
    ...
    def execute(self, context: Any, parameters: dict[str, Any] | None = None) -> ToolResult:
        ...
        evidence = self.detect(
            ...
            min_distinct_receivers=int(params.get("min_distinct_receivers", 5)),
            time_window_hours=float(params.get("time_window_hours", 24)),
            min_transaction_count=int(params.get("min_transaction_count", 5)),
        )
        return ToolResult(status=ExecutionStatus.SUCCESS, summary=f"Detected {len(evidence)} fan-out evidence item(s).", data={"evidence": evidence})
    ...
    def detect(
        ...
        *,
        ...
        candidate_slices = self._candidate_windows(
            ...
            account_column="sender_account",
            counterparty_column="receiver_account",
            window=window,
            min_distinct_counterparties=min_distinct_receivers,
            min_transaction_count=min_transaction_count,
        )
        evidence: list[DetectionEvidence] = []
        for account_id, slice_frame in candidate_slices:
            receiver_count = int(slice_frame["receiver_account"].nunique())
            transaction_count = int(len(slice_frame))
            total_amount = float(slice_frame["amount_paid"].sum())
            ...
            evidence.append(
                DetectionEvidence(
                    ...
                    involved_account_ids=self._bounded_unique(slice_frame["receiver_account"].tolist()),
                    entity_ids=self._bounded_unique(slice_frame.get("receiver_entity_id", pd.Series(dtype=object)).dropna().tolist()),
                    start_time=slice_frame["timestamp"].min().to_pydatetime(),
                    end_time=slice_frame["timestamp"].max().to_pydatetime(),
                    transaction_count=transaction_count,
                    total_amount=total_amount,
                    evidence_strength=min(1.0, receiver_count / float(min_distinct_receivers)),
                    severity=self._severity(receiver_count, min_distinct_receivers),
                    reasons=[
                        f"Source account sent funds to {receiver_count} distinct receivers within a {time_window_hours:g} hour window, meeting the configured threshold of {min_distinct_receivers}.",
                        f"{transaction_count} outgoing transactions totaled {total_amount:.2f}.",
                    ],
                    detector_parameters={
                        "min_distinct_receivers": min_distinct_receivers,
    ...
    def _prepare_transactions(self, transactions: pd.DataFrame) -> pd.DataFrame:
        ...
        if missing_columns:
            raise ValueError(f"Fan-out detection requires columns: {sorted(missing_columns)}")
    ...
    def _candidate_windows(
        ...
        *,
        ...
        outgoing = transactions.sort_values([account_column, "timestamp"]).reset_index(drop=True)
        candidates: list[tuple[str, pd.DataFrame]] = []
        for account_id, group in outgoing.groupby(account_column, sort=False):
            if group.empty:
                continue
